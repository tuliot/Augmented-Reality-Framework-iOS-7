...
#pragma mark Accelerometer
    ...
    if(![self CMaccelerometerData]){
        self.CMaccelerometerData = [[CMAccelerometerData alloc] init];
    }
    
    if (![self CMmotionManager]){
        NSOperationQueue *queue = [NSOperationQueue currentQueue];
        self.CMmotionManager = [[CMMotionManager alloc] init];
        self.CMmotionManager.deviceMotionUpdateInterval = INTERVAL_UPDATE;
...
- (void) updateViewAngle{
    ...
    CMAcceleration acceleration = self.CMmotionManager.accelerometerData.acceleration;
...
#pragma mark -
#pragma mark This is the problem right here!!!
	point.y = (realityBounds.size.height / 2) + (radianToDegrees(M_PI_2 + viewAngle)  * 3.0);
